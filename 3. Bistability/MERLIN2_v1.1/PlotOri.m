...
if isempty(options.EdgeColor)
    if ~isempty(Trigl)
         if ~isempty(options.DoubleFaceIDX)
         incolor= [147,112,219]/256 ;   % Original
         excolor= [255 193 193]./256;  
                patch('faces', Trigl( options.DoubleFaceIDX==1,:), 'vertices', Node, 'facecolor', excolor, ...
        ...
        hold on
                patch('faces', Trigl( options.DoubleFaceIDX==0,:), 'vertices', Node, 'facecolor', incolor, ...
            'linestyle', options.BendEdgeStyle, 'facelighting', 'flat',...
            'edgecolor', (1-options.EdgeShade)*[1 1 1], 'FaceVertexCData',options.FaceVertexColor);
        material dull
        else
    ...
    if strcmpi(options.ShowNumber,'on')
        for i=1:size(Node,1) 
            text(Node(i,1),Node(i,2),Node(i,3),num2str(i),'Fontsize',14);
        end
    end
...
IP.addParameter('PanelColor',[147,112,219]/256); %  [255 240 245]./256
